(() => {
  ...
  function renderOverviewProduct(readWeek, finishedWeek, conversion, stats) {
    ...
    return `<article class="panel"><div class="panel-head"><div><h3 class="section-title">Pulso do produto</h3><p class="section-copy">Solicitações registradas nos últimos 7 dias</p></div><button class="text-button" data-nav="analytics">Analisar tudo ${icon("arrow-right")}</button></div>
      ...
      <div class="notice ${asNumber(f.missed) > asNumber(f.finished) ? "notice--warning" : "notice--info"}" style="margin:17px 0 0">${icon(asNumber(f.missed) > asNumber(f.finished) ? "alert" : "activity")}<span>${esc(verdict)}</span></div>
    </article>`;
  }
  ...
  function renderServiceSummary() {
    if (state.loading.health && !state.health) return `<article class="panel skeleton" style="min-height:210px"></article>`;
  ...
  function renderUserResults() {
    ...
    const rows = list.map((account) => {
      ...
      return `<tr tabindex="0" data-action="open-user" data-open-user="${esc(account.key)}">
        ...
        <td>${pill(license.label, license.tone)}${account.isTester || account.isInternal ? `<div class="tags" style="margin-top:5px">${account.isTester ? '<span class="tag">testador</span>' : ""}${account.isInternal ? '<span class="tag">interna</span>' : ""}</div>` : ""}</td>
  ...
  function renderUserSummary(account, store) {
    ...
    if (store.loading.insights && insights === undefined) {
      insightBlock = `<section class="detail-section skeleton" style="min-height:190px"></section>`;
    ...
    return `${alertRows.length ? `<section class="detail-section"><h3>Pontos de atenção</h3>${alertRows.map(([tone, title, copy]) => `<div class="notice notice--${tone === "red" ? "danger" : tone === "yellow" ? "warning" : "info"}" style="margin:8px 0 0">${icon("alert")}<span><strong>${esc(title)}</strong><br>${esc(copy)}</span></div>`).join("")}</section>` : ""}${insightBlock}
  ...
  function renderServices() {
    ...
    return header + `<section class="hero-status" style="${problems.length ? "background:linear-gradient(130deg,rgba(255,107,112,.09),rgba(245,200,75,.035))" : ""}"><span class="hero-status-icon" style="${problems.length ? "background:var(--red-soft);color:var(--red)" : ""}">${icon(problems.length ? "alert" : "heart-pulse")}</span><div class="hero-status-copy"><span class="eyebrow">Estado geral</span><h3>${esc(overallTitle)}</h3><p>${esc(overallCopy)}</p></div><div class="hero-status-actions"><button class="button button--secondary button--compact" data-refresh-resource="health">${icon("refresh")} Verificar agora</button></div></section>
      <section class="service-grid" style="margin-top:15px">${services.map((service) => `<article class="service-card"><div class="service-card-head"><span class="service-icon" style="${service.ok && !service.zombie ? "background:var(--green-soft);color:var(--green)" : "background:var(--red-soft);color:var(--red)"}">${icon(service.icon)}</span><span class="service-name"><strong>${esc(service.name)}</strong><small>${service.ok && !service.zombie ? "Operacional" : service.zombie ? "Estado zumbi" : "Indisponível"}</small></span>${pill(service.ok && !service.zombie ? "OK" : "Atenção", service.ok && !service.zombie ? "green" : "red")}</div><p class="service-detail">${esc(serviceDescription(service))}</p><div class="info-list" style="margin-top:10px"><div class="info-row"><span>Latência</span><span>${service.latency_ms != null ? `${count(service.latency_ms)} ms` : "Não medida"}</span></div>${service.status_code != null ? `<div class="info-row"><span>HTTP</span><span>${esc(service.status_code)}</span></div>` : ""}${service.instance_name ? `<div class="info-row"><span>Instância</span><span>${esc(service.instance_name)}</span></div>` : ""}</div></article>`).join("")}</section>
      <div class="notice notice--info" style="margin-top:15px">${icon("activity")}<span>Este painel mostra apenas o estado atual. Ainda não há histórico de uptime persistido pelo backend.</span></div>`;
  }
  ...
  function renderWhatsapp() {
    ...
    return header + `<section class="hero-status" style="${connected ? "" : "background:linear-gradient(130deg,rgba(245,200,75,.09),rgba(91,155,255,.035))"}"><span class="hero-status-icon" style="${connected ? "" : "background:var(--yellow-soft);color:var(--yellow)"}">${icon("phone")}</span><div class="hero-status-copy"><span class="eyebrow">Conexão Evolution</span><h3>${esc(statusTitle)}</h3><p>${esc(statusCopy)}</p></div><div class="hero-status-actions">${configured && !connected ? `<button class="button button--primary button--compact" data-whatsapp-action="admin_connect">${icon("refresh")} Gerar pareamento</button>` : ""}${connected ? `<button class="button button--secondary button--compact" data-whatsapp-action="admin_logout">Desconectar</button>` : ""}</div></section>
      ${configured ? `<section class="content-grid" style="margin-top:15px"><article class="panel"><div class="panel-head"><div><h3 class="section-title">Configuração da instância</h3><p class="section-copy">Segredos completos nunca são exibidos</p></div>${pill(connected ? "Conectado" : "Desconectado", connected ? "green" : "yellow")}</div><div class="info-list"><div class="info-row"><span>Instância</span><span>${esc(config.instance_name || "—")}</span></div><div class="info-row"><span>Número operador</span><span>${esc(config.owner_number || "—")}</span></div><div class="info-row"><span>Endpoint</span><span>${esc(config.base_url || "—")}</span></div><div class="info-row"><span>Chave</span><span>${esc(config.api_key_masked || "mascarada")}</span></div><div class="info-row"><span>Atualizada</span><span>${esc(config.updated_at ? new Date(config.updated_at).toLocaleString("pt-BR") : "—")}</span></div></div><div class="detail-actions" style="margin-top:15px"><button class="button button--warning-soft button--compact" data-whatsapp-action="admin_reset">Reiniciar pareamento</button><button class="button button--danger-soft button--compact" data-whatsapp-action="admin_force_recreate">Recriar instância</button></div></article>
      <article class="panel"><div class="panel-head"><div><h3 class="section-title">Pareamento</h3><p class="section-copy">Use o WhatsApp do número operador</p></div></div><div class="qr-layout" style="grid-template-columns:180px 1fr"><div class="qr-frame">${qr ? `<img src="${esc(qr)}" alt="QR code para conectar o WhatsApp">` : `<div class="qr-placeholder">${icon("phone")}<p>${connected ? "Instância conectada" : "Gere um novo QR code"}</p></div>`}</div><div><h3 class="section-title">${connected ? "Conexão pronta" : "Abra Aparelhos conectados"}</h3><p class="section-copy">No WhatsApp, acesse Configurações → Aparelhos conectados e leia o código.</p>${payload.pairing_code ? `<span class="pairing-code">${esc(payload.pairing_code)}</span>` : ""}${payload.message ? `<div class="notice" style="margin:12px 0 0"><span>${esc(payload.message)}</span></div>` : ""}</div></div></article></section>` : `<div class="notice notice--warning" style="margin-top:15px">${icon("alert")}<span>A configuração EVOLUTION_BASE_URL, EVOLUTION_API_KEY e EVOLUTION_INSTANCE_NAME precisa ser concluída no servidor.</span></div>`}
      <section style="margin-top:15px">${renderVerificationMetrics()}</section>`;
  }
  ...
  function renderBroadcast() {
    ...
    return header + `${!connected ? `<div class="notice notice--warning">${icon("alert")}<span><strong>WhatsApp não confirmado como operacional.</strong> Verifique a conexão antes de enviar qualquer comunicado. <button class="text-button" data-nav="whatsapp" style="margin-left:5px">Abrir WhatsApp</button></span></div>` : ""}<section class="broadcast-layout">
      <article class="panel"><div class="panel-head"><div><h3 class="section-title">1. Mensagem</h3><p class="section-copy">Até 4.096 caracteres, enviados como texto</p></div><div class="mode-toggle"><button data-broadcast-mode="verified" aria-pressed="${b.mode === "verified"}">Verificados</button><button data-broadcast-mode="manual" aria-pressed="${b.mode === "manual"}">Lista manual</button></div></div>
        ...
        ${b.mode === "manual" ? `<label class="field-label" for="broadcast-manual">Números, um por linha</label><textarea id="broadcast-manual" placeholder="5511999999999\n5511888888888">${esc(b.manual)}</textarea><p class="section-copy">Espaços e pontuação são removidos. Duplicados são consolidados.</p>` : `<div class="notice notice--info" style="margin-top:15px">${icon("shield")}<span>Somente números que concluíram a verificação por WhatsApp entram nesta lista.</span></div>`}
        <div class="recipient-summary" style="margin-top:15px"><span class="attention-icon is-blue">${icon("users")}</span><span><strong id="recipient-count">${count(recipients.length)}</strong><small id="recipient-label">${b.mode === "verified" ? "números verificados" : "números válidos e únicos"}</small></span>${state.loading.recipients && b.mode === "verified" ? icon("loader", "icon spin") : ""}</div>
        ${state.errors.recipients && b.mode === "verified" ? `<div class="notice notice--danger" style="margin-top:12px">${icon("alert")}<span>${esc(state.errors.recipients)}</span></div>` : ""}
        <div class="detail-actions" style="margin-top:16px"><button id="broadcast-send" class="button button--primary" data-send-broadcast ${b.sending || !b.message.trim() || !recipients.length || !connected ? "disabled" : ""}>${b.sending ? icon("loader", "icon spin") : icon("send")} ${b.sending ? "Enviando…" : "Revisar e enviar"}</button>${b.mode === "verified" ? `<button class="button button--secondary" data-refresh-resource="recipients">${icon("refresh")} Atualizar destinatários</button>` : ""}</div>
        ${b.sending || b.results.length ? renderBroadcastProgress() : ""}
      </article>
      <aside class="panel"><div class="panel-head"><div><h3 class="section-title">2. Prévia</h3><p class="section-copy">Como o texto será entregue</p></div></div><div id="message-preview" class="message-preview">${b.message.trim() ? esc(b.message) : "Sua mensagem aparecerá aqui."}</div><div class="info-list" style="margin-top:15px"><div class="info-row"><span>Canal</span><span>WhatsApp</span></div><div class="info-row"><span>Destinatários</span><span id="preview-recipient-count">${count(recipients.length)}</span></div><div class="info-row"><span>Lotes previstos</span><span id="preview-batch-count">${count(Math.ceil(recipients.length / 5))}</span></div><div class="info-row"><span>Histórico</span><span>Não persistido</span></div></div><div class="notice" style="margin:15px 0 0">${icon("alert")}<span>O servidor atual não armazena campanhas. Recarregar a página durante o envio impede retomar os lotes restantes.</span></div></aside>
    </section>`;
  }
